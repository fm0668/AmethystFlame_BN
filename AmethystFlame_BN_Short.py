...
class GridTradingBot:
    ...
    def check_orders_status(self):
        ...
        for order in orders:
            ...
            # 判断订单类型
            if side == 'sell' and position_side == 'SHORT':  # 空头卖单
    ...
    async def run(self):
        """启动 WebSocket 监听"""
        # 初始化时获取一次持仓数据
        self.short_position = self.get_position()
        # self.last_position_update_time = time.time()
        logger.info(f"初始化持仓: 空头 {self.short_position} 张")
        ...
        logger.info(
            f"初始化挂单状态: 空头开仓={self.sell_short_orders}, 空头止盈={self.buy_short_orders}")
    ...
    async def handle_ticker_update(self, message):
        ...
        if data.get("e") == "bookTicker":  # Binance 的 bookTicker 事件
            ...
            # 检查持仓状态是否过时
            if time.time() - self.last_position_update_time > SYNC_TIME:  # 超过 SYNC_TIME 秒未更新
                self.short_position = self.get_position()
                self.last_position_update_time = time.time()
                logger.info(f"同步 position: 空头 {self.short_position} 张 @ ticker")

            # 检查挂单状态是否过时
            if time.time() - self.last_orders_update_time > SYNC_TIME:  # 超过 SYNC_TIME 秒未更新
                self.check_orders_status()
                self.last_orders_update_time = time.time()
                logger.info(f"同步 orders: 空头卖单 {self.sell_short_orders} 张, 空头买单 {self.buy_short_orders} 张 @ ticker")
        ...
        async with self.lock:
            ...
            if data.get("e") == "ORDER_TRADE_UPDATE":  # 处理订单更新
                ...
                if symbol == f"{self.coin_name}{self.contract_type}":  # 匹配交易对
                    ...
                    if status == "NEW":
                        if side == "SELL":
                            if position_side == "SHORT":  # 空头开仓单
                                self.sell_short_orders += remaining
                        elif side == "BUY":
                            if position_side == "SHORT":  # 空头止盈单
                    ...
                    # # 打印当前挂单状态
                    # logger.info(
                    #     f"挂单状态: 空头开仓={self.sell_short_orders}, 空头止盈={self.buy_short_orders}")
                    # # 打印当前持仓状态
                    # logger.info(f"持仓状态: 空头={self.short_position}")
    ...
    async def initialize_short_orders(self):
        # 检查上次挂单时间，确保 ORDER_FIRST_TIME 秒内不重复挂单
        current_time = time.time()
        ...
        # # 检查是否有未成交的挂单
        # orders = self.exchange.fetch_open_orders(self.ccxt_symbol)
        # if any(order['side'] == 'sell' and order['info'].get('positionSide') == 'SHORT' for order in orders):
        #     logger.info("发现未成交的空头补仓单，跳过撤销和挂单")
        #     return
    ...
    def place_take_profit_order(self, ccxt_symbol, side, price, quantity):
        ...
        for order in orders:
            if (
                    ...
                    and order['side'] == ('buy' if side == 'short' else 'sell')
            ):
        ...
        try:
            # 检查持仓
            if side == 'short' and self.short_position <= 0:
    ...
    async def place_short_orders(self, latest_price):
        ...
        try:
            ...
            if self.short_position > 0:
                # print('空头持仓', self.short_position)
                # 检查持仓是否超过阈值
                if self.short_position > POSITION_THRESHOLD:
                    print(f"持仓{self.short_position}超过极限阈值 {POSITION_THRESHOLD}，short装死")
                    if self.buy_short_orders <= 0:
                        logger.info("发现空头止盈单缺失。。需要补止盈单")
                        self.place_take_profit_order(self.ccxt_symbol, 'short', self.latest_price * 0.99,
    ...
    def check_and_reduce_positions(self):
        ...
        if self.short_position >= local_position_threshold:
            logger.info(f"空头持仓超过阈值 {local_position_threshold}，开始平仓，减少库存风险")
            # 平仓空头（使用市价单）
            if self.short_position > 0:
    ...
    async def adjust_grid_strategy(self):
        ...
        # print(self.latest_price, '空挂', self.sell_short_orders, '空平', self.buy_short_orders)

        # 检测空头持仓
        if self.short_position == 0:
            print(f"检测到没有空头持仓{self.short_position}，初始化空头挂单@ ticker")
